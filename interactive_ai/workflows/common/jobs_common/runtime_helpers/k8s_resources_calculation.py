...
def k8s_cpu_to_millicpus(k8s_cpu: str) -> int:
    """Convert CPU from k8s unit to millicpus."""
...
def k8s_memory_to_kibibytes(k8s_memory: str) -> int:
    """Convert memory from k8s unit to kibibytes."""
...
async def calculate_available_resources_per_node() -> tuple[list[int], list[int], list[int], list[int], str, str]:
    """Return node resources from compose-friendly environment variables."""
    available_cpu = [k8s_cpu_to_millicpus(os.environ.get("AVAILABLE_CPU", "8000m"))]
    available_memory = [k8s_memory_to_kibibytes(os.environ.get("AVAILABLE_MEMORY", "64Gi"))]
    cpu_capacity = [k8s_cpu_to_millicpus(os.environ.get("CPU_CAPACITY", "8000m"))]
    memory_capacity = [k8s_memory_to_kibibytes(os.environ.get("MEMORY_CAPACITY", "64Gi"))]
    accelerator_name = os.environ.get("ACCELERATOR_NAME", "nvidia.com/gpu")

    gpu_count = 0
    capacities_env = os.environ.get("GPU_CAPACITY", "").strip()
    if capacities_env:
        try:
            gpu_count = max((int(item.strip()) for item in capacities_env.split(",") if item.strip()), default=0)
        except ValueError:
            gpu_count = 0
    elif gpu_count_env := os.environ.get("GPU_COUNT", "").strip():
        try:
            gpu_count = max(0, int(gpu_count_env))
        except ValueError:
            gpu_count = 0

    accelerator_type = "gpu" if gpu_count > 0 else "cpu"
    return (available_cpu, available_memory, cpu_capacity, memory_capacity, accelerator_type, accelerator_name)
