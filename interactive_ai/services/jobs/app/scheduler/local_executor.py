...
LocalExecutor that:
  ...
  3. Publishes synthetic Kafka messages on the ``workflow_event`` topic on state transitions
...
_WORKFLOW_EXECUTION_EVENT_REQUEST = "com.geti.jobs.WorkflowExecutionEventRequest"

# Kafka topic consumed by ProgressHandler
_WORKFLOW_EVENT_TOPIC = "workflow_event"
...
class LocalExecutor(metaclass=Singleton):
    ...
    def _start_celery_execution(
        ...
        def _watch_result() -> None:
            try:
                timeout_sec = max(_DEFAULT_RESULT_TIMEOUT_SEC, int(_SIM_DURATION_SEC * 10))
                if job_type == "train":
    ...
    def _publish_workflow_event(self, record: _ExecutionRecord, phase: str) -> None:
        """
        Publish a synthetic Kafka message on ``workflow_event`` that follows the
        WorkflowExecutionEventRequest shape so the existing ProgressHandler picks it up.

        The message body follows the shape that ProgressHandler.on_workflow_event()
