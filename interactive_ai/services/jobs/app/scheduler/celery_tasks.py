...
def _docker_run_cmd() -> list[str]:
    ...
    workflow_ipc = os.environ.get("WORKFLOW_RUNTIME_IPC", "").strip()
    if workflow_ipc:
        cmd += ["--ipc", workflow_ipc]
    else:
        workflow_shm_size = os.environ.get("WORKFLOW_RUNTIME_SHM_SIZE", "").strip()
        if workflow_shm_size:
            cmd += ["--shm-size", workflow_shm_size]
...
def _append_trainer_runtime_memory_flags(cmd: list[str]) -> None:
    if "--ipc" in cmd or "--shm-size" in cmd:
        return

    ipc_mode = os.environ.get("TRAINER_RUNTIME_IPC", "").strip()
    if ipc_mode:
        cmd += ["--ipc", ipc_mode]
        return
...
def _run_optimize_trainer_container(payload: dict) -> None:
    ...
    _append_trainer_runtime_memory_flags(cmd)
