...
def _run_job_type(job_type: str, payload: dict) -> None:
    ...
    if job_type == "train":
        ...
        if stage == "evaluate":
            ...
            train_data = TrainWorkflowData.from_json(prep["train_data_json"])
            train_output_model_ids = TrainOutputModelIds.from_json(prep["train_output_model_ids_json"])
            train_ctx = TrainWorkflowDataForTaskTrainer(
                train_data=train_data,
...
def run() -> None:
    ...
    if not os.environ.get("WORKFLOW_TASK_ID"):
        if job_type == "train":
            task_id = (
                "job.tasks.evaluate_and_infer.evaluate_and_infer.evaluate_and_infer"
                if stage == "evaluate"
                else "train"
                if stage == "finalize"
                else "job.tasks.prepare_and_train.prepare_data_and_train.prepare_training_data_model_and_start_training"
            )
        elif job_type == "optimize_pot":
            task_id = (
                "job.tasks.evaluation_task.evaluate_optimized_model_pot"
                if stage == "evaluate"
                else "job.tasks.helpers.finalize_optimize"
                if stage == "finalize"
                else "job.tasks.optimization_task.shard_dataset_prepare_models_and_start_optimization"
            )
        else:
            task_id = f"compose.{job_type}"
        os.environ["WORKFLOW_TASK_ID"] = task_id
