...
@patch("policies.main.policy_executor.submit")
@patch("policies.main.resource_manager_executor.submit")
def test_initialize_compose_gpu_capacity_sets_default(mock_resource_manager_submit, mock_policy_submit, request):
    ...
    with patch.dict(os.environ, {"DEPLOYMENT_MODE": "compose"}):
        policy_main.DEPLOYMENT_MODE = "compose"
        policy_main.start()
...
@patch("policies.main.policy_executor.submit")
@patch("policies.main.resource_manager_executor.submit")
def test_initialize_compose_gpu_capacity_skips_if_already_set(
    mock_resource_manager_submit, mock_policy_submit, request
):
