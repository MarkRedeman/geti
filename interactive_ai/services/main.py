...
def handle_base_exception(request: Request, e: GetiBaseException) -> Response:
    ...
    if (
        e.http_status not in [200, 201, 202, 203, 205, 206, 207, 208, 226]
        and request.method == "GET"
    ):
        headers = {"Cache-Control": "no-cache"}
    if (
        e.http_status in [http.HTTPStatus.NO_CONTENT, http.HTTPStatus.NOT_MODIFIED]
        or e.http_status < 200
    ):
        return Response(status_code=int(e.http_status), headers=headers)
    return JSONResponse(
        content=response, status_code=int(e.http_status), headers=headers
    )
...
def _import_service_main(
    service_root: str,
    module_path: str,
    purge_prefixes: tuple[str, ...] = ("communication",),
):
...
try:
    ...
    _jobs_grpc_service_module = importlib.import_module(
        "microservice.grpc_api.grpc_job_service"
    )
finally:
    try:
...
GRPCJobService = _jobs_grpc_service_module.GRPCJobService
...
async def _auto_train_lifespan():  # noqa: ANN201
    ...
    thread = threading.Thread(
        target=_run_auto_train_loop, daemon=True, name="auto-train-controller"
    )
    thread.start()
    try:
        yield
        ...
        thread.join(
            timeout=int(_auto_train_main.AUTO_TRAIN_CONTROLLER_LOOP_INTERVAL) + 5
        )
...
def _start_jobs_grpc_process() -> Process:
    process = Process(target=GRPCJobService.serve)
    process.start()
    return process


def _stop_jobs_grpc_process(process: Process) -> None:
...
@asynccontextmanager
async def _jobs_grpc_lifespan():  # noqa: ANN201
    process = _start_jobs_grpc_process()
    try:
        yield
    finally:
        _stop_jobs_grpc_process(process)


@asynccontextmanager
async def _lifespan(app_instance: FastAPI):  # noqa: ANN201
    async with _model_registration_lifespan():
        async with _jobs_grpc_lifespan():
