...
def _validate_downloaded_artifact(filepath: str, url: str, content_type: str) -> None:
    with open(filepath, "rb") as file:
        head = file.read(2048)
    if "text/html" in content_type.lower() or _looks_like_html(head):
        raise RuntimeError(f"Downloaded HTML instead of model artifact from URL: {url}")
...
def download_file(url: str, target_path: str, auto_unzip=True):  # noqa: ANN001, ANN201, D103
    ...
    _validate_downloaded_artifact(download_temp_target_path, url, content_type)
...
def download_pretrained_model(model_spec: dict, target_dir: str, weights_url: str | None = None):  # noqa: ANN201, D103
    ...
    if weights_url is not None and "://" not in model_external_url:
