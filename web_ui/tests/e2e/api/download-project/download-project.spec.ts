...
const cardSegmentation = {
    ...
    //projectId: '67850c4efe167c785aea6b1a',
    // Rotated...
    //projectId: '67850c05dec95ec797df7442',

    // Fish
    projectId: '678f87b8c04f77fc54b9a165',

    // Card classification
    // projectId: '67850fe1548b266d6b2ae1bb',

    // Diopsis
    //projectId: '6790f2db6396992a0d533d9e',
...
async function importToDataset(
    ...
    if (isImage(mediaItem)) {
        const result = await instance.post(router.MEDIA_UPLOAD(datasetIdentifier, MEDIA_GROUP.IMAGES), formData, {
            headers: { 'content-type': 'multipart/form-data' },
        });

        const newMediaItem = getMediaItemFromDTO(datasetIdentifier, result.data, router);

        const mappedAnnotations = ((await annotations) as Annotation[]).map((annotation): Annotation => {
        ...
        if (isVideo(mediaItem)) {
            ...
            const result = await RetryablePromise.retry(3, () =>
                instance.post(router.MEDIA_UPLOAD(datasetIdentifier, MEDIA_GROUP.VIDEOS), formData, {
                    headers: { 'content-type': 'multipart/form-data' },
                })
            );
            ...
            for (const frameNumber of Object.keys(videoAnnotations)) {
                const annotations = videoAnnotations[Number(frameNumber)].map((annotation): Annotation => {
                    const labels = annotation.labels.map(mapLabelToProject);
                ...
                const labels = task.labels
                    ...
                    .map((label) => {
                        return {
                            ...
                            parentLabelId: label.parentLabelId === null ? null : labelIdToName[label.parentLabelId],
...
test('Download project dataset', async ({ apiServiceConfiguration, applicationServices, request }) => {
    ...
    await steps(
        ...
        async (dataset) => {
            ...
            const exportPath = resolveDatasetPath('exported', kebabCase(project.name), kebabCase(dataset.name));
            ...
            for await (const mediaItem of mediaItems) {
                await test.step(mediaItem.name, async () => {
                    ...
                    await storeMediaItem({ mediaItem, annotations, predictions, responsePromise }, exportPath);
