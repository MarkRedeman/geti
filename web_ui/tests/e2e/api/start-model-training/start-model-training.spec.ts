...
import { collect, filter, jobsIterator, map, projectsPagesIterator } from '../iterators';
...
test.only('Train models for every project', async ({ apiServiceConfiguration, applicationServices }) => {
    ...
    const projects = await projectsPagesIterator(workspaceIdentifier, applicationServices.projectService);
    ...
    for await (const project of projects) {
        ...
        await test.step('Start training for all model architectures', async () => {
            ...
            const jobs = jobsIterator(workspaceIdentifier, jobsService, projectIdentifier, [JobType.TRAIN]);
            ...
            for (const task of project.tasks) {
                ...
                const trainingJObModemTemplateIds = await collect(
                    map(
                        filter(jobs, (job) => isJobTrain(job) && job.metadata.task.taskId === task.id),
                        (job) => (isJobTrain(job) ? (job.metadata.task.modelTemplateId ?? '') : '')
                    )
                );

                const modelTemplateIdsToIgnore = new Set([
                    ...trainingJObModemTemplateIds,
    ...
    await test.step('Waiting for training job', async () => {
        ...
        while (isTraining) {
            const { jobs, jobsCount, nextPage } = await applicationServices.jobsService.getJobs(
                workspaceIdentifier,
                {
            ...
            console.log(activeJobs, jobsCount, jobs.length, nextPage);
            isTraining = Number(jobsCount.numberOfRunningJobs) + Number(jobsCount.numberOfScheduledJobs) > 0;
    ...
    for await (const project of projectsPagesIterator(workspaceIdentifier, applicationServices.projectService)) {
        const projectIdentifier = { ...workspaceIdentifier, projectId: project.id };
    ...
    await test.step('Waiting for optimization jobs to finish', async () => {
        ...
        while (isOptimizing) {
            ...
            isOptimizing = Number(jobsCount.numberOfRunningJobs) + Number(jobsCount.numberOfScheduledJobs) > 0;
    ...
    for await (const project of projectsPagesIterator(workspaceIdentifier, applicationServices.projectService)) {
        ...
        await test.step('Run tests for each model version', async () => {
            console.log('Getting model groups');
            const projectService = applicationServices.projectService;
    ...
    await test.step('Waiting for model testing jobs to finish', async () => {
        ...
        while (isModelTesting) {
            ...
            isModelTesting = Number(jobsCount.numberOfRunningJobs) + Number(jobsCount.numberOfScheduledJobs) > 0;
